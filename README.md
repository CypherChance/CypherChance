## TryHackMe Learnings

### Pre-Security Learning Path

| Room | Key Takeaways | Screenshot |
|------|----------------|-------------|
| [Offensive Security Intro](https://tryhackme.com/room/offensivesecurityintro) | In this room, I learned the 2 general sides in Cybersecurity, the Offensive and Defensive side. But this room focused more on the offensive side. Which is basically about compromising an existing system. In this room, I was able to hack into a fake bank, which somehow, gave me an idea of what it is like hacking a bank. It is really important to sanitize your company's website, because in this case, I was able to access the webpage meant for admins to access, which allowed me to exploit it. | ![Hacked The Fake Bank](PreSecurity/Intro-to-Offensive-Security/HackFakeBank.png) |
| [Defensive Security Intro](https://tryhackme.com/room/defensivesecurityintro) | In this room, I was introduced to what is defensive security, simply the counterpart of offensive security, it is all about protecting something, preventing malicious activities from happening, and also responding to any attacks happened. I was also able to experience a mini SIEM where I get to block specific IP using the built-in SIEM Firewall.| ![Computer Components Lab](PreSecurity/Intro-to-Defensive-Security/SIEMFirewall.png) |
| [Inside a Computer System](https://tryhackme.com/room/insideacomputer) | In this room, I was able to gain understanding how computer works, what are the core parts that makes up a computer, and its functions. I was also able to gain knowledge on the process of booting up a computer, it is important to understand because often times it is being targetted by the attacker.| ![Computer Components Lab](PreSecurity/Inside-A-Computer-System/ComputerComponents.png) |

### Complete Beginner Path (In Progress)

- [x] [How Websites Work](https://tryhackme.com/room/howwebsiteswork) – HTTP requests/responses, status codes.  
  ![Websites](screenshots/beginner/how-websites-work.png)

## Projects
- Detection Lab
- SOC Automation Project
